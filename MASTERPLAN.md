...
9. **Data wordt stuk per stuk heropgebouwd — met Maxim als eindcontrole per item.** (Aangescherpt 2026-08-02 bij Verse Start 2, Migratie 59.) De database is opnieuw leeggemaakt behalve sportpakketten (incl. planning), locaties, personeel en kampmomenten. Werkwijze thema-invoer: **Maxim overloopt zélf in de chat elk onderdeel/materiaal dat hij in de themabundel ziet staan**; PDF-extractie mag als voorbereiding dienen maar is nooit de bron van waarheid — pas na Maxims item-per-item-akkoord gaat iets de database in. Zo komen er geen fouten in de database.
